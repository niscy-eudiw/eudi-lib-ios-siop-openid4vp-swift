...
public struct SecCertificateHelper: Sendable {
  ...
  static func validateCertificateChain(certs: [SecCertificate]) -> Bool {
    ...
    let status = SecTrustCreateWithCertificates(chain as CFArray, policy, &maybeTrust)
    guard status == errSecSuccess, let trust = maybeTrust else {
      return false
    }
    
    SecTrustSetAnchorCertificates(trust, [rootCerts] as CFArray)
